...
public class HeadShowView : MonoBehaviour
{
    ...
    void Start ()
    {
		...
		if (keys != null) {
			keybuffer = new ComputeBuffer (computekeys.Length, sizeof(float));
			if (iscanvas) {
				gameObject.GetComponent<RawImage> ().material.SetBuffer ("key", keybuffer);
			} else {
